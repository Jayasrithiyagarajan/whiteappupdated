...
<title>Admin - Survey KPI Dashboard</title>

<!-- DataTables CSS (optional if we need a table later, but good for base styles) -->
...
<style>
/* ============================================================
   SURVEY KPI DASHBOARD (Premium UI)
   ============================================================ */
.akpi-main {
    padding: 24px;
...
.akpi-filter-bar input[type="date"]:focus {
    outline: none; border-color: #a78bfa; background: rgba(255,255,255,.12);
}
.btn-akpi-filter {
    padding: 9px 24px;
    background: linear-gradient(135deg, #7c3aed, #a78bfa);
    color: #fff; border: none; border-radius: 8px; font-size: 13px; font-weight: 700;
    cursor: pointer; transition: all .25s; box-shadow: 0 4px 14px rgba(124,58,237,.3);
}
.btn-akpi-filter:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(124,58,237,.4); }
.btn-akpi-reset {
    padding: 9px 20px;
...
@media (max-width: 900px) { .akpi-cards-row { grid-template-columns: 1fr; } }
...
@media (max-width: 1200px) { .akpi-chart-grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 900px) { .akpi-chart-grid { grid-template-columns: 1fr; } }
.akpi-chart-row-full { display: grid; grid-template-columns: 1fr; gap: 24px; margin-bottom: 24px; }
...
<div class="akpi-main">
    ...
    <div class="akpi-page-header">
        <div class="hdr-left">
            <h4><i class="fas fa-poll"></i>Customer Survey KPI Dashboard</h4>
            <p>Analysis of customer feedback and survey responses.</p>
        </div>
        <div class="akpi-filter-bar">
            <input type="date" id="flt_date_from" title="From Date">
            <input type="date" id="flt_date_to"   title="To Date">
            
            <button class="btn-akpi-filter" onclick="loadSurveyKPI()">
                <i class="fas fa-filter"></i> Apply
            </button>
            <button class="btn-akpi-reset" onclick="resetSurveyKPI()">
    ...
    <div class="akpi-cards-row">
        <div class="akpi-stat-card c-purple">
            <div>
                <div class="asc-label">Total Surveys</div>
                <div class="asc-value" id="val_surveys">0</div>
            </div>
            <div class="asc-icon bg-purple"><i class="fas fa-file-contract"></i></div>
        </div>
        <div class="akpi-stat-card c-emerald">
            <div>
                <div class="asc-label">New Clients</div>
                <div class="asc-value" id="val_new">0</div>
            </div>
            <div class="asc-icon bg-emerald"><i class="fas fa-user-plus"></i></div>
        </div>
        <div class="akpi-stat-card c-blue">
            <div>
                <div class="asc-label">Existing Clients</div>
                <div class="asc-value" id="val_existing">0</div>
            </div>
            <div class="asc-icon bg-blue"><i class="fas fa-user-check"></i></div>
        </div>
    </div>

    <!-- 5 PIE CHARTS FOR QUESTIONS -->
    <div class="akpi-chart-grid">
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-stopwatch"></i><h6>Response Time Performance</h6></div>
            </div>
            <div id="chart_rt" style="min-height:300px"></div>
        </div>
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-hard-hat"></i><h6>PPE Adherence</h6></div>
            </div>
            <div id="chart_ppe" style="min-height:300px"></div>
        </div>
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-id-card"></i><h6>Qualification Card</h6></div>
            </div>
            <div id="chart_qual" style="min-height:300px"></div>
        </div>
    </div>
    ...
    <div class="akpi-chart-grid">
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-clipboard-check"></i><h6>Aramco Standards</h6></div>
            </div>
            <div id="chart_aramco" style="min-height:300px"></div>
        </div>
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-smile"></i><h6>Overall Satisfaction</h6></div>
            </div>
            <div id="chart_sat" style="min-height:300px"></div>
        </div>
    </div>

    <!-- Monthly Trend -->
    <div class="akpi-chart-row-full">
        <div class="akpi-card-box">
            <div class="acb-header">
                <div class="acb-header-left"><i class="fas fa-chart-line"></i><h6>Surveys Submitted Over Time</h6></div>
            </div>
...
<script>
var reqApi = null;
var chrtRt, chrtPpe, chrtQual, chrtAramco, chrtSat, chrtTrend;
var isClearing = false;

var CKPI = {
    purple: '#8b5cf6', emerald: '#10b981', blue: '#3b82f6', orange: '#f97316',
...
function renderNoDataMsg(sel) {
    $(sel).html('<div class="akpi-no-data"><i class="fas fa-chart-pie"></i><span>No data available</span></div>');
}

$(document).ready(function() {
    loadSurveyKPI();

    $('#flt_date_from, #flt_date_to').on('change', function() {
        if (!isClearing) loadSurveyKPI();
    });
});

function resetSurveyKPI() {
    isClearing = true;
    $('#flt_date_from, #flt_date_to').val('');
    isClearing = false;
    loadSurveyKPI();
}

function loadSurveyKPI() {
    var params = {
    ...
    if (reqApi && reqApi.readyState !== 4) reqApi.abort();
    reqApi = $.ajax({
        url: 'fetch_survey_kpi_data.php',
        type: 'GET',
        data: params,
        dataType: 'json'
    }).done(function(res) {
        ...
        var k = res.kpi || {};
        initAnimNumber('val_surveys', k.total || 0);
        initAnimNumber('val_new', k.new || 0);
        initAnimNumber('val_existing', k.existing || 0);

        var d = res.distributions || {};
        drawPieChart(d.response_time, '#chart_rt', 'Response Time');
        drawPieChart(d.ppe, '#chart_ppe', 'PPE');
        drawPieChart(d.qualification, '#chart_qual', 'Qualification');
        drawPieChart(d.aramco, '#chart_aramco', 'Aramco Standards');
        drawPieChart(d.satisfaction, '#chart_sat', 'Satisfaction');

        drawTrendChart(res.trend || {});
...
// ── Generic Pie Builder ──
function drawPieChart(d, sel, nameFallback) {
    // Map existing chart references depending on selector to allow clean teardowns
    if (sel === '#chart_rt') chrtRt = safeDestroy(chrtRt, sel);
    else if (sel === '#chart_ppe') chrtPpe = safeDestroy(chrtPpe, sel);
    else if (sel === '#chart_qual') chrtQual = safeDestroy(chrtQual, sel);
    else if (sel === '#chart_aramco') chrtAramco = safeDestroy(chrtAramco, sel);
    else if (sel === '#chart_sat') chrtSat = safeDestroy(chrtSat, sel);

    var total = (d.data || []).reduce((a,b) => a+b, 0);
    if(total === 0) { renderNoDataMsg(sel); return; }

    var options = {
        chart: { type: 'pie', height: 300, fontFamily: 'inherit' },
        series: d.data, labels: d.labels,
        colors: [CKPI.emerald, CKPI.blue, CKPI.amber, CKPI.rose, CKPI.purple, CKPI.sky],
    ...
    if (sel === '#chart_rt') chrtRt = chart;
    else if (sel === '#chart_ppe') chrtPpe = chart;
    else if (sel === '#chart_qual') chrtQual = chart;
    else if (sel === '#chart_aramco') chrtAramco = chart;
    else if (sel === '#chart_sat') chrtSat = chart;
}

// ── Trend Column/Bar Chart ──
function drawTrendChart(d) {
    chrtTrend = safeDestroy(chrtTrend, '#chart_trend');
    if(!d.labels || !d.labels.length) { renderNoDataMsg('#chart_trend'); return; }

    chrtTrend = new ApexCharts(document.querySelector('#chart_trend'), {
        chart: { 
            type: 'bar', 
            height: 320, 
            toolbar: { show: false }, 
        ...
        series: [{ name: 'Surveys Submitted', data: d.data }],
        xaxis: { 
            categories: d.labels,
