...
<div class="container-fluid mt-4">
    ...
    <div class="directory-hero">
        <div class="directory-title">
            <h2>Eddy Current Inspection Registry</h2>
            <p>Monitor eddy current inspection certificates, expiry status, clients, inspectors, and inspection dates.</p>
        </div>
        ...
        <div class="filter-row">
            <div class="filter-item">
                ...
                <select id="filter-inspector" class="form-select">
                    ...
                    <?php 
                    $inspectors = $conn->query("SELECT DISTINCT inspector FROM eddy_current_inspection WHERE inspector != '' ORDER BY inspector");
            ...
            <div class="filter-item">
                ...
                <select id="filter-client" class="form-select">
                    ...
                    <?php 
                    $clients = $conn->query("SELECT DISTINCT customer_name FROM eddy_current_inspection WHERE customer_name != '' ORDER BY customer_name");
            ...
            <div class="filter-item">
                ...
                <select id="filter-year" class="form-select">
                    ...
                    <?php 
                    $years = $conn->query("SELECT DISTINCT YEAR(inspection_date) as y FROM eddy_current_inspection ORDER BY y DESC");
                    while($row = $years->fetch_assoc()) echo "<option value='{$row['y']}'>{$row['y']}</option>";
                    ?>
                </select>
            </div>
            <div class="filter-item">
                <button class="btn-clear" type="button" onclick="clearEddyFilters()">
    ...
    <div class="card-box">
        <div class="table-panel-header">
            <div class="table-title">
                <h5>Eddy Current Certificate Register</h5>
                <p>Search, sort, export, filter, and manage inspection records.</p>
            </div>
        ...
        <div class="table-shell">
            <table id="eddy-table" class="display nowrap" style="width:100%">
                <thead>
                    <tr>
                        <th>Project No</th>
...
$(document).ready(function() {
    var table = $('#eddy-table').DataTable({
        processing: true,
        serverSide: true,
        deferRender: true,
        scrollX: true,
        ...
        order: [[7, 'desc']], // inspection_date DESC
        dom: 'Brtip',
        buttons: [
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    var form = $('<form>', {
                        action: 'fetch_eddy_current_certificates.php',
                        method: 'POST'
                    });
        ...
        ajax: {
            url: 'fetch_eddy_current_certificates.php',
            type: 'POST',
            data: function(d) {
...
function clearEddyFilters() {
    $('.form-select, .form-control, #customSearch').val('');
    $('#eddy-table').DataTable().search('');
    $('#eddy-table').DataTable().ajax.reload();
}
