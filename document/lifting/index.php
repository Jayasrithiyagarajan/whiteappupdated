...
<title>Lifting Gear Inspection Registry</title>
...
<div class="container-fluid mt-4">
    ...
    <div class="directory-hero">
        <div class="directory-title">
            <h2>Lifting Gear Inspection Registry</h2>
            <p>Monitor lifting gear certificates, active and expired items, clients, inspectors, and exam dates.</p>
        </div>
        ...
        <div class="filter-row">
            <div class="filter-item">
                ...
                <select id="filter-inspector" class="form-select">
                    ...
                    <?php 
                    $inspectors = $conn->query("SELECT DISTINCT inspector FROM lifting_gear_certificates WHERE inspector != '' ORDER BY inspector");
            ...
            <div class="filter-item">
                ...
                <select id="filter-client" class="form-select">
                    ...
                    <?php 
                    $clients = $conn->query("SELECT DISTINCT customer_name FROM lifting_gear_certificates WHERE customer_name != '' ORDER BY customer_name");
            ...
            <div class="filter-item">
                ...
                <select id="filter-year" class="form-select">
                    ...
                    <?php 
                    $years = $conn->query("SELECT DISTINCT YEAR(date_of_this_examination) as y FROM lifting_gear_certificates ORDER BY y DESC");
                    while($row = $years->fetch_assoc()) echo "<option value='{$row['y']}'>{$row['y']}</option>";
                    ?>
                </select>
            </div>
            <div class="filter-item">
                <button class="btn-clear" type="button" onclick="clearLiftingFilters()">
    ...
    <div class="card-box">
        <div class="table-panel-header">
            <div class="table-title">
                <h5>Lifting Gear Records</h5>
                <p>Manage lifting gear certificates with inspection details and expiry tracking</p>
            </div>
        ...
        <div class="table-shell">
            <table id="lifting-table" class="display nowrap" style="width:100%">
                <thead>
                    <tr>
                        <th>Project No</th>
...
<script>
var liftingTable = $('#lifting-table').DataTable({
    processing: true,
    serverSide: true,
    scrollX: true,
    pageLength: 10,
    ...
    buttons: [
        {
            ...
            action: function (e, dt, node, config) {
                ...
                var form = $('<form>', {
                    action: 'fetch_lifting_certificates.php',
                    method: 'POST'
                });
    ...
    ajax: {
        url: 'fetch_lifting_certificates.php',
        type: 'POST',
        data: function(d) {
...
$('#customSearch').on('keyup', function() {
    liftingTable.search(this.value).draw();
});

$('.form-select, .form-control').on('change keyup', function() {
    liftingTable.ajax.reload();
});
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    ...
                    if (res.success) {
                        $('#lifting-table').DataTable().ajax.reload();
                    } else {
